...
namespace AeccBot.Utils
{
    ...
    public class MessagingUtils
    {
        ...
        {
            Party party = new PartyWithTimestamps(activity.ServiceUrl, activity.ChannelId, activity.From, activity.Conversation);

            var userParties = _routingDataManager.GetUserParties();
            int index = userParties.IndexOf(party);

            if (index != -1)
            {
                party = userParties[index];
                _routingDataManager.UpdatePartyConnectionData(party, activity);
            }
            return party;
        }
        ...
        public static Party GetReferenceParty(Party partyBulk)
        {
            Party party = null;

            var userParties = _routingDataManager.GetUserParties();
            int index = userParties.IndexOf(partyBulk);
        ...
        public static async Task ReplyToActivityAsync(Activity activity, string message)
        {
            ...
            {
                Debug.WriteLine($"Either the activity is null or the message is empty - Activity: {activity}; message: {message}");
            }
        }
        ...
        /// the message and the message activity (the content of the message).</returns>
        public static ConnectorClientAndMessageBundle CreateConnectorClientAndMessageActivity(
            Party partyToMessage, string messageText, ChannelAccount senderAccount, string eventText = null)
        {
            ...
            {
                newMessageActivity.Type = ActivityTypes.Event;
                newMessageActivity.Text = eventText;
                newMessageActivity.ChannelData = messageText;
            }
