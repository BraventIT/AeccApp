...
namespace AeccApp.Core.Services
{
    public class HospitalRequestService : IHospitalRequestService
    {
        ...
        public async Task<IList<Hospital>> GetHospitalsAsync(string province, CancellationToken cancelToken)
        {
            UriBuilder uribuilder = new UriBuilder(GlobalSetting.Instance.ApiEndpoint)
            {
                Path = "api/Hospitals",
            ...
            var identityToken = await IdentityService.GetTokenAsync();
            return await _requestProvider.GetAsync<IList<Hospital>>(uribuilder.ToString(), cancelToken, identityToken);
        }
